"use client";
import type { Message, ErrorResponse, MessageStatus } from "@/shared/types/chat";
...
export function ChatbotMessage({ message }: { message: Message }) {
  ...
  const renderMessageContent = () => {
    ...
    // Loading state: status is "writing" but no content yet
    if (message.status === "writing" && message.content === "") {
      return <ThinkingIndicator />;
    }

    // Writing state: status is "writing" and content exists (streaming)
    // Complete state: status is "complete" or undefined
